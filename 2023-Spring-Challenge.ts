    ...
    while (true) {
        ...
        myBases.forEach(base => {
            actions.push(["LINE", base, findNearestResourceCell(base).id, 1].join(' '));
        })
    ...
    function findNearestResourceCell(base: number): Cell {
        const queue: number[] = ([] as any).concat(cells[base].neighbors);
        const visited = new Set();
        visited.add(base);
        let result: number;
        while (result == null) {
            const currentCell = queue.shift();
            if (cells[currentCell].resources > 0) {
                result = currentCell;
            } else {
                visited.add(currentCell);
                queue.push(...cells[currentCell].neighbors.filter(n => !visited.has(n)));
            }
        }
        console.error("base", base, "nearest crystal cell", result);
        return cells[result];
    }